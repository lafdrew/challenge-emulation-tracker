| Challenge | 厂商 | 型号 | 漏洞类型 | 漏洞数量 | 是否模拟 |
|---|---|---|---|---|---|
| PREAUTH-RCE-001 | Tenda | AC15 AC1900 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-002 | Wavlink | WL-WN530HG4 | 认证前RCE | 1 | 待模拟 |
| PREAUTH-RCE-003 | D-Link | DIR-816 A2 | 认证前RCE | 7 | 已模拟 |
| PREAUTH-RCE-004 | D-Link | DIR-882 | 认证绕过 | - | 已模拟 |
| PREAUTH-RCE-005 | ASUS | GT-AC2900 | 认证绕过 | - | 已模拟 |
| PREAUTH-RCE-006 | NETGEAR | WAC104 | 认证绕过 | - | 已模拟 |
| PREAUTH-RCE-007 | Buffalo | WSR-2533DHPL2/WSR-2533DHP3 | 认证绕过 | - | 已模拟 |
| PREAUTH-RCE-008 | NETGEAR | Nighthawk R8300 | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-009 | NETGEAR | Nighthawk R7000 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-010 | NETGEAR | Nighthawk R7000 China-only release | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-011 | NETGEAR | Orbi RBR20 | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-012 | NETGEAR | RAX30 | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-013 | NETGEAR | RAX30 | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-014 | NETGEAR | Nighthawk R6700v3 | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-015 | NETGEAR | Nighthawk R6700v3 | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-016 | D-Link | DIR-878 Rev A | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-017 | TP-Link | Archer A7/C7 V5 | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-018 | TP-Link | Archer C7 V5 / AC1750 | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-019 | Synology | BC500 IP Camera | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-020 | Western Digital | My Cloud Pro PR4100 | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-021 | Western Digital | My Cloud Pro PR4100 | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-022 | QNAP | QHora-322 | 认证前RCE | - | 已模拟 |
| PREAUTH-RCE-023 | Tenda | AC18 AC1900 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-024 | Tenda | AC6 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-025 | Tenda | AC9 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-026 | Tenda | AC9 V3.0 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-027 | Tenda | AC6 V2.0 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-028 | NETGEAR | Nighthawk R6400 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-029 | NETGEAR | Nighthawk R6900 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-030 | D-Link | DIR-859 A1 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-031 | TOTOLINK | N600R | 认证前RCE | 3 | 待模拟 |
| PREAUTH-RCE-032 | D-Link | DIR-645 A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-033 | D-Link | DIR-815 A1 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-034 | D-Link | DIR-816L | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-035 | D-Link | DIR-822 C1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-036 | NETGEAR | Nighthawk R8000 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-037 | D-Link | DIR-868L A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-038 | D-Link | DIR-860L A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-039 | D-Link | DIR-880L A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-040 | NETGEAR | Nighthawk R6700 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-041 | D-Link | DIR-890L A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-042 | D-Link | DIR-850L A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-043 | D-Link | DIR-818LW A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-044 | D-Link | DIR-885L A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-045 | NETGEAR | R6250 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-046 | NETGEAR | Nighthawk X6 R7900 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-047 | D-Link | DIR-817LW A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-048 | D-Link | DIR-865L A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-049 | D-Link | DIR-869 A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-050 | D-Link | DIR-895L A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-051 | NETGEAR | D6400 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-052 | NETGEAR | Nighthawk R7300DST | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-053 | NETGEAR | R7100LG | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-054 | D-Link | DAP-1650 A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-055 | D-Link | DIR-412 A1 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-056 | D-Link | DAP-1522 B1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-057 | D-Link | DAP-1525 A1 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-058 | D-Link | DIR-300 A1 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-059 | D-Link | DIR-300 Bx | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-060 | D-Link | DIR-320 A1 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-061 | D-Link | DIR-600 B5/B6 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-062 | D-Link | DIR-845L A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-063 | D-Link | DIR-518L A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-064 | D-Link | DIR-803 A1 | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-065 | NETGEAR | D6220 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-066 | NETGEAR | Nighthawk X8 R8500 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-067 | TRENDnet | TEW-751DR | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-068 | TRENDnet | TEW-733GR | 认证前RCE | 2 | 已模拟 |
| PREAUTH-RCE-069 | D-Link | DIR-842 C1 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-070 | NETGEAR | Nighthawk X4S R6400v2 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-071 | TOTOLINK | A3002R | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-072 | TOTOLINK | A3002R V4 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-073 | D-Link | DIR-868L B1 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-074 | TOTOLINK | A3002RU | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-075 | D-Link | DIR-860L B1 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-076 | TOTOLINK | A3002RU V3 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-077 | D-Link | DIR-868L B1 2.01B05 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-078 | D-Link | DIR-860L B1 2.01B03 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-079 | D-Link | DIR-860L B1 2.03B03 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-080 | D-Link | DIR-868L B1 2.03B01 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-081 | D-Link | DIR-880L A1 1.02B13 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-082 | D-Link | DIR-880L A1 1.01B09 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-083 | D-Link | DIR-890L A1 1.03B07 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-084 | D-Link | DIR-860L A1 1.00 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-085 | D-Link | DIR-860L A1 1.01 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-086 | D-Link | DIR-860L A1 1.04b02 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-087 | D-Link | DIR-860L A1 1.07B01 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-088 | D-Link | DIR-860L A1 1.08B02 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-089 | D-Link | DIR-860L A1 1.09B06 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-090 | D-Link | DIR-860L A1 1.10B04 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-091 | D-Link | DIR-868L A1 1.01B05 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-092 | D-Link | DIR-868L A1 1.02B03 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-093 | D-Link | DIR-868L A1 1.04 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-094 | D-Link | DIR-868L A1 1.06B01 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-095 | D-Link | DIR-868L A1 1.07B02 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-096 | D-Link | DIR-868L A1 1.08B07 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-097 | D-Link | DIR-868L A1 1.09B08 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-098 | D-Link | DIR-890L A1 1.06B04 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-099 | D-Link | DIR-817LW A1 1.00B05 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-100 | D-Link | DIR-817LW A1 1.02B01 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-101 | D-Link | DIR-865L A1 1.00B24 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-102 | D-Link | DIR-865L A1 1.02B05 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-103 | D-Link | DIR-865L A1 1.03B02 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-104 | D-Link | DIR-865L A1 1.05B07 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-105 | D-Link | DIR-865L A1 1.08B01 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-106 | D-Link | DIR-850L A1 1.04b03 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-107 | D-Link | DIR-850L A1 1.05b03 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-108 | D-Link | DIR-850L A1 1.06b05 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-109 | D-Link | DIR-850L A1 1.07b01 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-110 | D-Link | DIR-850L A1 1.10B08 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-111 | D-Link | DIR-850L A1 1.12B05 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-112 | D-Link | DIR-850L A1 1.13B01 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-113 | D-Link | DIR-859 A1 1.03B04 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-114 | D-Link | DAP-1650 A1 1.02B02 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-115 | D-Link | DIR-865L A1 1.04B022 Facebook | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-116 | D-Link | DIR-865L A1 1.06B06 Beta | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-117 | D-Link | DIR-803 A1 1.02B01 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-118 | D-Link | DIR-803 A1 1.03B06 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-119 | D-Link | DIR-803 A1 1.04B02 | 认证前RCE | 1 | 已模拟 |
| PREAUTH-RCE-120 | D-Link | DIR-890L A1 1.06B01 HOTFIX | 认证前RCE | 1 | 已模拟 |
